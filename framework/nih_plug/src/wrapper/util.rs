...
impl ScopedFtz {
    fn enable() -> Self {
        #[cfg(not(miri))]
        {
            ...
            {
                // Use inline assembly to access MXCSR register directly
                // FTZ is bit 15 in the MXCSR register
                let ftz_bit: u32 = 1 << 15;
                let mut csr: u32 = 0;
                ...
                let should_disable_again = csr & ftz_bit == 0;

                if should_disable_again {
                    // Set the FTZ bit
                    let new_csr = csr | ftz_bit;
                    unsafe {
                        std::arch::asm!(
                            "ldmxcsr [{}]",
                            in(reg) &new_csr,
...
impl Drop for ScopedFtz {
    fn drop(&mut self) {
        ...
        if self.should_disable_again {
            #[cfg(target_feature = "sse")]
            {
                // Use inline assembly to clear the FTZ bit
                let ftz_bit: u32 = 1 << 15;
                let mut csr: u32 = 0;
