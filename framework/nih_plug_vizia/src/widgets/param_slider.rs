    ...
    fn event(&mut self, cx: &mut EventContext, event: &mut Event) {
        ...
        event.map(|window_event, meta| match window_event {
            ...
            | WindowEvent::MouseTripleClick(MouseButton::Left) => {
                // If the previous drag was never completed (e.g. because the
                // mouse left the window while the button was held down and the
                // `MouseUp` event was never delivered), clean up the stale drag
                // state first. Without this the slider stays permanently
                // captured and the GUI appears frozen.
                //
                // We intentionally do NOT start a new interaction here: this
                // event was routed to us only because we still held the capture.
                // The next click will be routed to the correct entity.
                if self.drag_active {
            ...
            | WindowEvent::MouseTripleClick(MouseButton::Right) => {
                // Same stale-drag recovery as above.
                if self.drag_active {
                    self.drag_active = false;
                    cx.release();
                    cx.set_active(false);
